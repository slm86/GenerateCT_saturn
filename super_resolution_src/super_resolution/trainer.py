...
class SuperResolutionTrainer(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        assert (
            not SuperResolutionTrainer.locked
        ), "SuperResolutionTrainer can only be initialized once per process - for the sake of distributed training, you will now have to create a separate script to train each unet (or a script that accepts unet number as an argument)"
        assert exists(superres) ^ exists(
            superres_checkpoint_path
        ), "either superresolution instance is passed into the trainer, or a checkpoint path that contains the superresolution config"
        ...
        self.accelerator.init_trackers(
            project_name="superres-training",
            config=kwargs,
        )
        ...
        for ind, (
            ...
            zip(self.superres.unets, lr, eps, warmup_steps, cosine_decay_max_steps)
        ):
            optimizer = Adam(
                unet.parameters(),
                lr=unet_lr,
    ...
    def print_untrained_unets(self):
        ...
        for ind, (steps, unet) in enumerate(
            zip(self.steps.tolist(), self.superres.unets)
        ):
    ...
    def add_train_dataset(self, ds=None, *, batch_size, **dl_kwargs):
        ...
        self.add_valid_dataset(
            valid_ds, batch_size=batch_size, shuffle=True, **dl_kwargs
        )
    ...
    def train_step(self, unet_number=None, **kwargs):
        ...
        self.accelerator.log({"train/loss": loss})
    ...
    def valid_step(self, **kwargs):
        ...
        with context():
            ...
            self.accelerator.log({"valid/loss": loss})
        return loss
    ...
    def step_with_dl_iter(self, dl_iter, **kwargs):
        ...
        dl_tuple_output[2] = [
            dl_tuple_output[2] for i in range(dl_tuple_output[0].shape[0])
        ]
        ...
        model_input = dict(
            list(zip(self.dl_tuple_output_keywords_names, dl_tuple_output_casted))
        )
    ...
    def save(self, path, overwrite=True, without_optim_and_sched=False, **kwargs):
        ...
        save_obj = dict(
            model=self.superres.state_dict(), steps=self.steps.cpu(), **kwargs
        )
        ...
        if hasattr(self.superres, "_config"):
            self.print(
                f'this checkpoint is commandable from the CLI - "superresolution --model {str(path)} "<prompt>""'
            )
    ...
    def load(self, path, only_model=False, strict=True, noop_if_not_exist=False):
        ...
        try:
            ...
            self.superres.load_state_dict(
                restore_parts(self.superres.state_dict(), loaded_obj["model"])
            )
    ...
    def forward(self, *args, unet_number=None, max_batch_size=15, **kwargs):
        unet_number = self.validate_unet_number(unet_number)
        self.validate_and_set_unet_being_trained(unet_number)
        self.set_accelerator_scaler(unet_number)
