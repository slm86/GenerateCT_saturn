...
class Superres(nn.Module):
    ...
    def p_mean_variance(
        ...
        *,
        ...
        ), "superres was not trained with conditional dropout, and thus one cannot use classifier free guidance (cond_scale anything other than 1)"
    ...
    def sample(
        ...
        ), "text or text encodings must be passed into superres if specified"
        assert not (
            not self.condition_on_text and exists(text_embeds)
        ), "superres specified not to be conditioned on text, yet it is presented"
        assert not (
        ...
        frames_to_resize_kwargs = lambda frames: (
            dict(target_frames=frames) if exists(frames) else dict()
        )
