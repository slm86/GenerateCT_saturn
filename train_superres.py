...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--uname", type=str, default="", help="unique name for experiment"
    )
    ...
    unet1 = NullUnet()
    unet2 = [
        Unet(**v, lowres_cond=(i > 0)) for i, v in enumerate(config.unets.values())
    ]

    superres_klass = (
        ElucidatedSuperres if config.superres.get("elucidated", False) else Superres
    )
    ...
    while True:  # let slurm handle the stopping
        ...
        if cur_step % config.checkpoint.save_every_x_it == 0:
            ...
            if trainer.is_main:
                ...
                nib.save(
                    ...
                    os.path.join(
                        save_folder,
                        "images",
                        f"sample-{str(cur_step).zfill(10)}.nii.gz",
                    ),
                )
                affine = np.eye(4)  # example affine matrix
                ...
                nib.save(
                    ...
                    os.path.join(
                        save_folder,
                        "images",
                        f"sample-{str(cur_step).zfill(10)}-input.nii.gz",
                    ),
                )
                ...
                nib.save(
                    ...
                    os.path.join(
                        save_folder,
                        "images",
                        f"sample-{str(cur_step).zfill(10)}-real.nii.gz",
                    ),
                )
                filename = os.path.join(
                    save_folder, "images", f"sample-{str(cur_step).zfill(10)}.txt"
                )
            ...
            ckpt_path = os.path.join(
                save_folder, "models", f"{str(cur_step).zfill(10)}-ckpt.pt"
            )
