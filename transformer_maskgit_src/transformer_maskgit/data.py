...
class VideoDataset(Dataset):
    def __init__(
        ...
        for ext in exts:
            for p in Path(f"{folder}").rglob(f"*.{ext}"):
                if p.is_file():
                    if ext == "nii.gz":
                        # STEFI: this is a hack to handle the off chance case that the data has over 3 dimensions
                        if len(list(nib.load(p).dataobj.shape)) == 3 and (
    ...
    def nii_img_to_tensor(self, path, transform):
        ...
        except:  # STEFI: this is a hack to handle the case where the json file is not present (TODO: check if INSPECT data is already in HU)
            slope = 1
            intercept = 0
            manufacturer = ""
